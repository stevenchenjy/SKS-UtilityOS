    ...
    try:
        if database.is_file() and not database.is_symlink():
            with sqlite3.connect(database.resolve().as_uri() + '?mode=ro', uri=True, timeout=2) as db:
                ...
                schema = version if version in {1, 2, 3, 4, 5, 6, SCHEMA_VERSION} else None
                result['database'] = 'accessible' if db.execute('PRAGMA quick_check').fetchone()[0] == 'ok' else 'invalid'
                result['migration'] = 'current' if version == SCHEMA_VERSION else 'explicit_upgrade_available' if version in {1, 2, 3, 4, 5, 6} else 'unsupported'
