...
Only one pending correction per original is created through the application. If the original is cancelled or superseded after a draft was opened, that draft cannot be approved; reopen the current invoice or reject the draft. Closed drafts cannot be edited. Every financial approval requires literal Boolean `acknowledge: true` at the service boundary, including invoices with no warnings. HTTP approvals must explicitly carry the current draft revision as a nonnegative integer. Missing, null, Boolean, string, fractional, negative and stale revisions are rejected; failed approval creates no bill and preserves saved review state. Trusted internal synthetic seeding can omit a revision; that convention is not available through HTTP.
