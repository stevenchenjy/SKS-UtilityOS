import {showAcquisition} from './modules/acquisition.js';
...
const views={acquisition:showAcquisition,usage:showUsage,providers:showProviders,inbox:showInbox,completeness:showCompleteness,overview:showOverview,bills:showBills,review:showReview,inventory:showInventory,intervals:showIntervals,support:showSupport};
const nav=[['overview','Overview'],['bills','Invoice ledger'],['inbox','Utility Inbox'],['acquisition','Acquisition'],['providers','Provider Management'],['completeness','Bill completeness'],['review','Review queue'],['inventory','Utility inventory'],['intervals','Interval data'],['usage','Mapped usage files'],['support','Privacy & support']];
...
function login(){
  document.title='SKS UtilityOS | Local utility ledger';
  delete context.overviewFilters;delete context.billsFilters;
  $('#app').innerHTML=`<main class="login-screen" tabindex="-1"><div class="login-card"><div class="brand-symbol">${icon('inventory')}</div><h1>SKS UtilityOS</h1><p class="login-subtitle">A local home for campus utility bills.</p>${meta.mode==='demo'?notice('Synthetic demonstration. Every building, meter, and amount in this workspace is fictional.','warning'):notice('Staff workspace. Your local app passphrase is separate from all utility portal credentials.')}
  <form id="login-form">${meta.mode==='demo'?'':`<label class="field"><span>Local app passphrase</span><input type="password" id="password" name="password" autocomplete="current-password" required minlength="12" maxlength="256"></label>`}<div id="login-error" aria-live="polite"></div><button class="primary wide" type="submit">${meta.mode==='demo'?'Open synthetic demo':'Unlock local workspace'}</button></form><p class="caption">Version ${esc(meta.version)} · Runs on this computer · No portal access</p></div></main>`;
...
async function refresh(next,arg,focusMain=false){
  ...
  try{await views[next](target,context,arg);if(generation===renderGeneration)document.title=`${nav.find(n=>n[0]===next)?.[1]||'Ledger'} | SKS UtilityOS`;}
    ...
    renderBusy=false;$$('[data-nav]').forEach(button=>button.disabled=false);
    if(target.isConnected&&generation===renderGeneration){
      const restored=retainedControl&&(retainedControl.id?document.getElementById(retainedControl.id):retainedControl.name?$(`[name="${CSS.escape(retainedControl.name)}"]`,target):null);
      (restored||target).focus();
    }
  }
}
async function openStage(id){
  const target=$('#content');
  try{await showStage(target,context,id);target.focus();window.scrollTo(0,0);}
  catch(error){handleError(error,target);}
}
async function openProviderSetup(documentId=null,options={}){
  ...
  try{await showProviderSetup(target,context,documentId,options);document.title='Provider Setup | SKS UtilityOS';target.focus();window.scrollTo(0,0);}
  catch(error){handleError(error,target);}
}
...
function openImport(){
  ...
  $('#import-form').onsubmit=async e=>{
    ...
    button.disabled=true;button.textContent='Importing…';input.disabled=true;
    ...
    if(files.length===1&&results[0].count){closeDialog();toast(`${results[0].count} item(s) added for review.`);if(results[0].usage_ids?.length)await navigate('usage',results[0].usage_ids[0]);else if(results[0].count===1)await openStage(results[0].staged_ids[0]);else await navigate('review');}
    else if(files.length===1){target.innerHTML=notice(message(results[0].code),'danger');button.disabled=false;button.textContent='Import for review';input.disabled=false;input.focus();}
