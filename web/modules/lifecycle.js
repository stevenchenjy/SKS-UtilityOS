...
export function bindLifecycle(item,ctx,collect){
  ...
  if(item.status==='pending'){
    $('#save-draft').onclick=async()=>{try{await api(`/staged/${item.id}/draft`,{method:'POST',body:{payload:collect(),revision:item.revision,correction_of:item.correction_of,reason:item.correction_reason}});toast('Draft saved locally.');await ctx.openStage(item.id);}catch(e){fail(e);}};
