...
export async function showReview(container,ctx) {
  ...
  render('pending');
  $$('[data-filter]').forEach(button=>button.onclick=()=>{$$('[data-filter]').forEach(b=>b.classList.toggle('active',b===button));render(button.dataset.filter);});
